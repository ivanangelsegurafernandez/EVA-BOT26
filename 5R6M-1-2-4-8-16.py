    ...
    try:
        if bool(globals().get("HUD_MARTINGALA_LIVE_ENABLE", True)):
            ...
            try:
                ...
                pass
            try:
                alive_age = time.time() - float(globals().get("KEYBOARD_LISTENER_ALIVE_TS", 0.0) or 0.0)
                if alive_age > 5:
    ...
    try:
        ...
        if _manual_confirm_active():
            ...
            lines.append(
                ...
                f"🚨 TECLA MANUAL | BOT={bot} | CICLO=C{ciclo} | RESTAN={restante}s"
                + Style.RESET_ALL
            )
            lines.append(
                Fore.YELLOW + Style.BRIGHT +
                "👉 ¿INVERTIR EN REAL? [Y/S]=SÍ | [N/ESC]=NO"
                + Style.RESET_ALL
            )
            return lines

        if PENDIENTE_FORZAR_BOT:
            bot = str(PENDIENTE_FORZAR_BOT).upper()
            restante = max(0, int(float(PENDIENTE_FORZAR_EXPIRA or 0) - now))
            lines.append(
                Fore.MAGENTA + Style.BRIGHT +
                f"🎯 MANUAL REAL ARMADO | BOT={bot} | PRESIONA CICLO [1..{MAX_CICLOS}] | ESC=CANCELAR | {restante}s"
                + Style.RESET_ALL
            )
            return lines
        ...
        if st.get("active") and now <= float(st.get("expires") or 0):
            ...
            if err:
                lines.append(Fore.RED + Style.BRIGHT + f"⛔ MANUAL REAL | {err}" + Style.RESET_ALL)
            elif msg:
                lines.append(Fore.CYAN + Style.BRIGHT + f"⌨️ MANUAL REAL | {msg}" + Style.RESET_ALL)
        return lines
    except Exception:
        return []
...
def escuchar_teclas():
    ...
    global PENDIENTE_FORZAR_BOT, PENDIENTE_FORZAR_INICIO, PENDIENTE_FORZAR_EXPIRA, KEYBOARD_LISTENER_ALIVE_TS
        ...
        try:
            ...
            if HAVE_MSVCRT and msvcrt.kbhit():
                ...
                try:
                    ...
                    continue
                _manual_key_audit(f"key_read={repr(k)} confirm={_manual_confirm_active()} pending_bot={PENDIENTE_FORZAR_BOT}")
                ...
                if _manual_confirm_active():
                    ...
                    if k in ("y", "s"):
                        ...
                        _manual_key_audit(f"confirm_yes key={k} bot={bot_conf} ciclo={ciclo_conf}")
                        _clear_manual_confirm()
                        ok = forzar_real_manual(str(bot_conf), int(ciclo_conf))
                        _manual_key_audit(f"confirm_result ok={ok} bot={bot_conf} ciclo={ciclo_conf}")
                        if ok:
                            _manual_status_set(
                                "orden_enviada",
                                bot=str(bot_conf),
                                ciclo=int(ciclo_conf),
                                msg=f"ORDEN REAL ENVIADA | BOT={str(bot_conf).upper()} | CICLO=C{ciclo_conf}",
                                ttl_s=12,
                                last_key=k,
                            )
                            threading.Thread(
                                target=_manual_verify_real_order,
                                args=(str(bot_conf), int(ciclo_conf), 1.0),
                                daemon=True,
                            ...
                            _manual_status_set(
                                ...
                                bot=str(bot_conf),
                                ciclo=int(ciclo_conf),
                                error=f"NO SE ENVIÓ ORDEN REAL | BOT={str(bot_conf).upper()} | CICLO=C{ciclo_conf}",
                                ttl_s=15,
                                last_key=k,
                            )
                        _safe_render_keyboard_panel()
                    ...
                    if k in ("n", "\x1b"):
                        ...
                        _manual_key_audit(f"confirm_cancel key={repr(k)} bot={bot_conf} ciclo={ciclo_conf}")
                        _manual_status_set(
                            "cancelado",
                            bot=str(bot_conf),
                            ciclo=int(ciclo_conf),
                            msg=f"Cancelado BOT={str(bot_conf).upper()} C{ciclo_conf}",
                            ttl_s=6,
                            last_key=k,
                        )
                        _clear_manual_confirm()
                ...
                if k == "s":
                    ...
                    PENDIENTE_FORZAR_EXPIRA = 0.0
                    agregar_evento(
                        f"🎯 TECLA {k} PRESIONADA: {bot_sel.upper()} seleccionado para REAL en C{ciclo}. "
                        f"Confirma Y/S o cancela N/ESC."
                    )
