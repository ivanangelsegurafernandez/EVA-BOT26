...
class MonitorSaldoApp:
    ...
    def _on_plot_hover(self, event):
        ...
        try:
            ...
            dt = datetime.fromtimestamp(self._ts_raw[idx], tz=LIMA_TZ).strftime("%H:%M:%S") if hasattr(self, "_ts_raw") and idx < len(self._ts_raw) else "?"
    ...
    def actualizar_grafica(self):
        ...
        # MODO LÍNEA REAL: eje X por índice uniforme (estilo gráfica financiera)
        _valid = [(s.ts_epoch, float(s.saldo))
                  for s in window if s.saldo is not None]
        if not _valid:
            self._render_pause_banner()
            self.canvas.draw_idle()
            self.root.after(900, self.actualizar_grafica)
        ...
        _ts_raw = [v[0] for v in _valid]
        ys = [v[1] for v in _valid]
        ...
        self._xs_num = [float(x) for x in xs]
        self._ys_live = list(ys)
        self._ts_raw = _ts_raw
        ...
        # Tick labels: mostrar hora real en posiciones uniformes
        n_ticks = min(8, max(2, len(xs)))
        if len(xs) > 1:
            tick_idx = sorted(set(int(round(i * (len(xs) - 1) / max(n_ticks - 1, 1))) for i in range(n_ticks)))
        else:
            tick_idx = [0]
        self.ax.set_xticks([xs[i] for i in tick_idx])
        self.ax.set_xticklabels(
            [datetime.fromtimestamp(_ts_raw[i], tz=LIMA_TZ).strftime("%H:%M:%S") for i in tick_idx],
            rotation=18,
        )
